...
describe('MemoryWorkoutTemplatesRepo', () => {
  ...
  it('should retrieve a workout template by ID', async () => {
    ...
    expect(fetchedWorkoutTemplate?.name).toBe(workoutTemplate.name);
  });
  ...
  it('should delete all workout templates for a user', async () => {
    const workoutTemplate2 = workoutTemplateTestProps.createTestWorkoutTemplate(
      {
        id: 'template-2',
      },
    );

    const workoutTemplate3 = workoutTemplateTestProps.createTestWorkoutTemplate(
      {
        id: 'template-3',
        userId: 'user-2',
      },
