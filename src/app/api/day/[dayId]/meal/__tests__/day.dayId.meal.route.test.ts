...
describe("POST /api/day/[dayId]/meal", () => {
  ...
  it("returns JSEND format if user is not logged in", async () => {
    const response = await POST(
      new Request(`http://localhost/api/day/${testDayId}/meal`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ recipeIds: [testRecipe.id] }),
      }) as NextRequest,
  ...
  it("should add meals from recipes to day and return 201", async () => {
    ...
    const response = await POST(
      new Request(`http://localhost/api/day/${testDayId}/meal`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ recipeIds: [testRecipe.id, testRecipe.id] }),
      }) as NextRequest,
      { params: Promise.resolve({ dayId: testDayId }) },
    );
    ...
    expect(data.data.mealIds).toHaveLength(2);
  });
    ...
    it("should return 404 if recipe does not exist", async () => {
      ...
      const response = await POST(
        new Request(`http://localhost/api/day/${testDayId}/meal`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ recipeIds: ["non-existent-recipe"] }),
        }) as NextRequest,
        { params: Promise.resolve({ dayId: testDayId }) },
      );
