...
describe("Bool", () => {
  ...
  it("should throw ValidationError if value is not a boolean", () => {
    ...
    // @ts-expect-error testing invalid input
    expect(() => Bool.create("true")).toThrow(/(boolean|booleano)/i);
  });
  ...
  it("should throw ValidationError if value is null", () => {
    ...
    // @ts-expect-error testing invalid input
    expect(() => Bool.create(null)).toThrow(/(required|obligatorio)/i);
  });
  ...
  it("should throw ValidationError if value is undefined", () => {
    ...
    // @ts-expect-error testing invalid input
    expect(() => Bool.create(undefined)).toThrow(/(required|obligatorio)/i);
  });
