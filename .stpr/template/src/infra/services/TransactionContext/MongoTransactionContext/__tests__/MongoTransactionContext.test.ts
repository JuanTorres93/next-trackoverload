...
import {
  ...
  setupMongoTestDB,
  teardownMongoTestDB,
} from '@/infra/repos/mongoose/__tests__/setupMongoTestDB';
    ...
    it('should rollback transaction on error', async () => {
      try {
        await transactionContext.run(async () => {
          ...
          await TestModel.create([{ _id: 'doc1', name: 'Test 1', value: 100 }], { session });
    ...
    it('should maintain separate sessions for concurrent transactions', async () => {
      ...
      await Promise.all([
        transactionContext.run(async () => {
          ...
          await TestModel.create([{ _id: 'doc1', name: 'Transaction 1', value: 1 }], {
            session: session1,
          });
        }),
        new MongoTransactionContext().run(async () => {
          ...
          await TestModel.create([{ _id: 'doc2', name: 'Transaction 2', value: 2 }], {
            session: session2,
          });
        }),
      ]);
